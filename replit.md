...
- **SEO técnico & indexação por IA:** robots.txt expandido (User-agent específico para 16 bots de IA: GPTBot, ChatGPT-User, ClaudeBot, PerplexityBot, Google-Extended, CCBot, Applebot-Extended, etc.), sitemap.xml com lastmod/changefreq + namespaces image e xhtml (alternate `.md`), `/llms.txt` e `/llms-full.txt` (gerados via `server/llms.ts`), espelho `/<slug>.md` em Markdown via Turndown para LLMs/RAG, SSR meta tags + JSON-LD (`server/seo-html.ts`) com `BlogPosting` + `BreadcrumbList` + `FAQPage`, IndexNow + Google sitemap-ping automático ao publicar/atualizar posts (`server/indexnow.ts`), campo FAQ por post (coluna `posts.faq` JSON) e aba "SEO Técnico" no dashboard (Database > SEO Técnico) para verificações Google/Bing, OG default, texto e FAQ globais do `llms.txt`, e botão de reenvio em massa para IndexNow.
